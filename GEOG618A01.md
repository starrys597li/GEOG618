...
In a database management system, a view is a virtual table derived from one or more existing tables. Views can be used to simplify data access and management by presenting a customized subset of data from one or more tables. Views are also used to provide a layer of abstraction between the database schema and application logic, enabling changes to the underlying data structures without affecting the application. Views can also enforce data security by controlling access to specific columns or rows of data.

Views can be created for different purposes such as to restrict data access to certain users or groups, to simplify the queries required to retrieve data, to combine data from multiple tables, to mask sensitive data and so on. Views can also be used to provide a specific perspective on the data by hiding or revealing certain columns or aggregating data in a certain way. Overall, views are a powerful tool for managing complex databases and enabling efficient and secure data access for users and applications.
...
```sql
SELECT id, 43.37441226789755, -80.96862697511344,
    ( 3959 * acos( cos( radians(lat) ) * cos( radians(43.37441226789755) )
    * cos( radians(-80.96862697511344) - radians(lon) ) + sin( radians(lat) )
    * sin( radians(43.37441226789755) ) ) ) AS distance
FROM ways_vertices_pgr
ORDER BY distance ASC
LIMIT 1;
```
![image](https://user-images.githubusercontent.com/64757199/221750490-b445a00d-6ce4-4b89-8ebd-c7d3f7eac04d.png)
